...
class Day11(Solution):
    ...
    def part2(self, data):
        ...
        @lru_cache(maxsize=None)
        def dfs(node, found_dac, found_fft):
            if node == "out":
                return 1 if found_dac and found_fft else 0
            if node not in graph:
                return 0

            new_dac = found_dac or (node == "dac")
            new_fft = found_fft or (node == "fft")

            total = 0
            for neighbor in graph[node]:
                total += dfs(neighbor, new_dac, new_fft)
            return total
    ...
    def run(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(description=f'Run Day 11 solutions')
